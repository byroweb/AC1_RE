...
by adding `0x8000_0000` (e.g. his `0x001A26B8` = our `0x801A26B8`). It is an independent
RE (BizHawk RAM-watch vs. our Ghidra + DuckStation), so agreement is real cross-checking,
not a shared source.

## AC entity array (in-mission)

- **Base = `0x801A26B8`** — entity index 0 = the **player AC**. **CONFIRMED** (both:
  our player-transform trace, `s3 = 0x801A26B8`; Zinfidel `ENTITY_DATA`).
- **Stride = `0x170` (368 B)**. **CONFIRMED**: enemy AC index 1 sits at `0x801A2828`
  (`= base + 0x170`, our method-table-zero live test) and Zinfidel `ENTITY_OFFSET = 0x170`.
...
| --- | --- | --- | --- |
| `+0x00` | ptr → MT/instance template table (`0x8019FAB8`) | u32 | CONFIRMED |
...
> **Position offset to reconcile (open):** our trace reads the authoritative position as
